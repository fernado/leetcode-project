 ...
 */
public class TwoStringsPAndQ {

    public static void main(String[] args) {
        TwoStringsPAndQ twoStringsPAndQ = new TwoStringsPAndQ();

        System.out.println(twoStringsPAndQ.solution("abc", "bcd"));
        //System.out.println(twoStringsPAndQ.solution("axxz", "yzwy"));
        //System.out.println(twoStringsPAndQ.solution("bacad", "abada"));
        //System.out.println(twoStringsPAndQ.solution("amz", "amz"));
    }
    ...
    public int solution(String P, String Q) {
        ...
        for (int i = 0; i < P.length(); i++) {
            distinct.add(P.charAt(i));
            distinct.add(Q.charAt(i));
        }

        int distinctCount = P.length() + Q.length();
        for (char c1 : distinct) {
            distinctCount--;
            for (char c2 : distinct) {
                int count = 0;
                for (int i = 0; i < P.length(); i++) {
                    if (P.charAt(i) == c1 || Q.charAt(i) == c1 ||
                            P.charAt(i) == c2 || Q.charAt(i) == c2) {
                        count++;
                    }
                }
